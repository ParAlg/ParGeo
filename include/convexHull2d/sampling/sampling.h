...
namespace pargeo {
  namespace hull2d {
	...
	using floatT = typename pointT::floatT;
	floatT d = pow(floatT(m), floatT(0.5));
	size_t di = size_t(ceil(d));

	if (di > 2000000) // blows up size_t
	  throw std::runtime_error("too many grid samples");

	std::atomic<floatT> pMin[2];
	std::atomic<floatT> pMax[2];
	for (int i = 0; i < 2; ++ i) pMin[i] = std::numeric_limits<floatT>::max();
	for (int i = 0; i < 2; ++ i) pMax[i] = std::numeric_limits<floatT>::lowest();
	...
	floatT maxSpan = 0;
	for (int i = 0; i < 2; ++ i) maxSpan = std::max(maxSpan, pMax[i]-pMin[i]);
