...
static void randInc_inSphere_3d_10m(benchmark::State& state) {
  auto P = inSphere_float<3>(N);
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_onSphere_3d_10m(benchmark::State& state) {
  auto P = onSphere_float<3>(N);
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_inCube_3d_10m(benchmark::State& state) {
  auto P = inCube_float<3>(N);
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_onCube_3d_10m(benchmark::State& state) {
  auto P = onCube_float<3>(N);
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_armadillo_3d_173k(benchmark::State& state) {
  auto P = armadillo_3d_173k_float();
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_dragon_3d_438k(benchmark::State& state) {
  auto P = dragon_3d_438k_float();
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_buddha_3d_544k(benchmark::State& state) {
  auto P = buddha_3d_544k_float();
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_thaiStatue_3d_3_5m(benchmark::State& state) {
  auto P = thaiStatue_3d_3_5m_float();
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_asianDragon_3d_3_6m(benchmark::State& state) {
  auto P = asianDragon_3d_3_6m_float();
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}

static void randInc_lucy_3d_3_14m(benchmark::State& state) {
  auto P = lucy_3d_3_14m_float();
  for (auto _ : state) pargeo::hull3d::parallelQuickHull::compute(parlay::make_slice(P), state.range(0));}
...
BENCHMARK(randInc_dragon_3d_438k)->Unit(benchmark::kMillisecond)->Arg(maxThreads);
